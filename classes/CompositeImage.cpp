...
CompositeImage::CompositeImage(int parts, std::string path) {
    original = vips::VImage::new_from_file(path.c_str());
    compute_avg();
...
float CompositeImage::get_distance_to_img(CompositeImage* img2) {
    color c1 = get_avg_color();
    color c2 = img2->get_avg_color();
...
color CompositeImage::image_average(vips::VImage image) {
    color c;
    vips::VImage stats = image.bandmean();
