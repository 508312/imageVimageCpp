...
SDLGuimage::SDLGuimage( int w, int h, int row, int col, int detail_thresh, int local_transition_thresh,
    ...
    mLocalTransitionZoom = mCompositeImage->getWidth()/calculateSmallX();
...
void SDLGuimage::createDetailed() {
    ...
    for (int i = min_y_ind; i < max_y_ind; i++) {
        texrect.y = std::max(0.0f, std::round((i*theoretical_y - mCamMinY) * mZoom));
        texrect.h = (std::min((i+1)*theoretical_y, mCamMaxY) - std::max(mCamMinY, i * theoretical_y)) * mZoom;
        texrect.h += ((int)(texrect.y + texrect.h) != (int) std::max(0.0f, std::round(((i+1)*theoretical_y - mCamMinY) * mZoom)));

        y_start = std::max((mCamMinY - i*theoretical_y) * mZoom, 0.0f);
        y_end = std::min((mCamMaxY - i*theoretical_y)*mZoom, theoretical_y*mZoom);

        for (int j = min_x_ind; j < max_x_ind; j++) {
            texrect.x = std::max(0.0f, std::round((j*theoretical_x - mCamMinX) * mZoom));
            texrect.w = (std::min((j+1)*theoretical_x, mCamMaxX) - std::max(mCamMinX, j * theoretical_x)) * mZoom;
            texrect.w += ((int)(texrect.x + texrect.w) != (int) std::max(0.0f, std::round(((j+1)*theoretical_x - mCamMinX) * mZoom)));

            x_start = std::max((mCamMinX - j*theoretical_x) * mZoom, 0.0f);
...
void SDLGuimage::addNextImages(int min_x_ind, int min_y_ind, int max_x_ind, int max_y_ind) {
    ...
    float new_zoom = (theoretical_x * mZoom)/mCompositeImage->getWidth();
    double new_x, new_y;
    CompositeImage* img;
    ...
    for (int i = min_y_ind; i < max_y_ind; i++) {
        for (int j = min_x_ind; j < max_x_ind; j++) {
            ...
            new_x = (mCamX - j*theoretical_x) * mLocalTransitionZoom;
            new_y = (mCamY - i*theoretical_y) * mLocalTransitionZoom;

            mNextImages[mNextImages.size() - 1].changezoom(new_zoom);
...
void SDLGuimage::generateImage() {
    ...
    if (mShouldBeDetailed) {
        ...
        SDL_Rect rect{std::max(std::round(-mCamMinX * mZoom), 0.0f),
